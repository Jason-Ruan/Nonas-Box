...
class MyStuffVC: UIViewController {
    ...
    private typealias MenuOptionCell = MyStuffOptionCollectionViewCell
    ...
    private lazy var myStuffButtonsCollectionView: UICollectionView = {
        let cv = UICollectionView(scrollDirection: .vertical,
                                  ...
                                  shouldInset: true)
        cv.register(MenuOptionCell.self, forCellWithReuseIdentifier: MenuOptionCell.identifier)
        cv.dataSource = self
        cv.delegate = self
        return cv
    }()
...
extension MyStuffVC: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: MenuOptionCell.identifier, for: indexPath) as? MenuOptionCell else { return UICollectionViewCell() }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        guard let cell = collectionView.cellForItem(at: indexPath) as? MenuOptionCell else { return }
